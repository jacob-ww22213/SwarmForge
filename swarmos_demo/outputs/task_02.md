...
- 为代码类任务准备一个固定格式输出，方便后续接 CI 或 PR 流程。
...
- 增加一个 smoke test 命令，保证每次改动后都能快速验证主流程。
- 为代码类任务准备一个固定格式输出，方便后续接 CI 或 PR 流程。
...
## Final Output
这个 demo 应该把目标收敛为一个能跑通的系统样机：用少量专家展示路由、协作、批判和聚合四个核心机制，并通过 trace 与基线对比证明它不仅仅是一个更花哨的工作流。 当前任务带有工程属性，因此展示结果时最好同时给出结构化建议和下一步实现动作。 这次被路由到的专家是 Planner, Coding Engineer, Research Scientist, Product Strategist，足以体现角色分工。
